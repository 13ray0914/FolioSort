...
<title>FolioSort Curation</title>
<style>
...
<header><b>FolioSort curation</b><select id="paper"></select><button id="reload">Reload</button><button id="rebuildNow">Rebuild affected network</button><span id="rebuildState" class="small"></span><span id="status" class="small"></span></header>
<div class="layout"><main>
<div class="panel"><h2>Publication metadata</h2><div class="tabnote">Correct title, author names, publication year, journal, or DOI when automatic metadata retrieval is incomplete. The imported metadata remains unchanged; edits are stored as an append-only overlay.</div><div id="metadataOriginal" class="original"></div><div class="grid"><div class="field full"><label>Title</label><input id="metaTitle"></div><div class="field"><label>Publication year</label><input id="metaYear" inputmode="numeric" maxlength="4" placeholder="e.g. 2024"></div><div class="field"><label>Journal</label><input id="metaJournal"></div><div class="field full"><label>DOI</label><input id="metaDoi" placeholder="10.xxxx/..."></div><div class="field full"><label>Authors — one per line; “Family, Given” is recommended</label><textarea id="metaAuthors" class="authors"></textarea></div><input id="metaReason" class="full" placeholder="Reason for metadata correction (recommended)"><button id="metaSave" class="accent">Save metadata correction</button><button id="metaRestore">Restore imported metadata</button></div><div id="metaPreview" class="preview"></div><div class="notice" style="margin-top:7px">Saved curation is materialized immediately. FolioSort automatically queues a rebuild of every Literature Network project that contains this paper, so corrected author/year labels and curated terms/claims propagate to the graph without rerunning the heavy PDF/LLM pipeline.</div></div>
            ...
            try:
                with log_path.open("a", encoding="utf-8") as log:
                    ...
                    result = subprocess.run(
                        [str(py), str(script), "--project", slug],
                        cwd=str(self.root),
...
class Handler(BaseHTTPRequestHandler):
    server_version = "LiteratureCuration/4.3.0"
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/health":
            self.send_json({"ok": True, "version": "4.3.0-security-hardening"})
            return
