...
function openAccordion(name){const details=document.querySelector(`details[data-section="${name}"]`);if(!details)return;document.querySelectorAll('details.section').forEach(x=>{if(x!==details)x.open=false;});details.open=true;details.scrollIntoView({block:'nearest'});}
function setupAccordions(){const all=[...document.querySelectorAll('details.section')];all.forEach(d=>d.open=false);all.forEach(d=>d.addEventListener('toggle',()=>{if(!d.open)return;all.forEach(other=>{if(other!==d)other.open=false;});}));}
